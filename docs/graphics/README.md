...
- 全屏/输入已验证检查点为 `7d5e787`，产品版本 `1.3.2` / API 23；原分支
...
- 重连回归已确认两代 DXVK 各 40/40 帧序和 Modern 视频正常结束；GL/音频
  功能短测完成。生命周期测试新定位到[隐藏桌面的 GL 错误忙循环](gl-background-backoff.md)，
  已完成限速候选及 GL 32/64 位各五轮缩放/后台恢复，不能把功能通过当成
  无错误、无资源增长或十分钟完整稳定性通过。
...
5. 公共测试入口已支持显式 HAP 哈希、只读预检和跳过安装，去除旧工程包名/
   ...
   18:31/18:52 的旧 smoke Want 实际未启动 probe；公共 core 已迁移到正常
   game 入口并复测。后台失败重试候选只解决忙循环；生产/消费暂停协调、
   长稳/资源门槛与偶发 0x505 仍按新证据继续验收。
