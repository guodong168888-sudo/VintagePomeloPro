...
初始批次仅包含文档/契约锁；后续已推进配置模型、公共几何、DXVK 和呈现候选。
当前进展以 [入口状态](README.md) 和 [GLES 验证记录](gles-direct-validation.md)
为准。不要把下文历史 1.2.9 打包结果理解为当前设备或产物状态。

2026-08-31 收口仍需区分：稳定的 1.3.2 全屏/输入修复；默认关闭的 GLES Direct
候选；已撤回运行库覆盖并暂放的 War3 专项。当前设备缺少 native-buffer EGL
扩展，不能跑出真实 GLES Direct 性能门槛。公共回归入口已开始清理旧机器路径，
跨后端、五次生命周期、十分钟稳定性及 EGL 偶发 0x505 仍需逐项补证据，
不能将编译成功或一个游戏提速当作全部计划完成。
