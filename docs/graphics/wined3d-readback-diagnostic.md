...
## 18:14–18:29：游戏自身 OpenGL 局内短测与最终处理

用户随后要求完成一次有界局内短测再放下 War3。两侧均使用上述生产基线 HAP，
原 WineD3D/Guest Mesa，无诊断 DLL 覆盖；正常游戏 Want 配合既有
`observe-product-summary`。唯一游戏启动参数差异是 `-opengl`；
`batchMappedFlush` 始终 product/on。没有重建、安装或修改游戏文件。

- D3D8：`war3-d3d8-final-ab-20260831-181557`，14 个完整窗口、1,680 帧，
  59.777115 秒；呈现均值 28.1044 FPS，Presenter CPU P95 669 us，
  发布间隔 P95/P99 55.284/68.182 ms；本次工具状态 `MEASURED`。
- 自身 OpenGL：第一次重启后用户报告卡住，再次重启后菜单/局内可见。
  用户观察约增加 10 FPS。`war3-native-gl-final-ab-20260831-182929`
  包含 21 个完整窗口、2,520 帧，60.77155 秒；仅作描述性统计为 41.4668 FPS，
  Presenter CPU P95 540 us，发布间隔 P95/P99 30.630/38.108 ms。
  **工具状态仍为 `INCONCLUSIVE`，不是性能或正确性 PASS。**
- 18:29:08.840，NCP `blit dropped serial=22833 gl=0x505 egl=0x3000 restore=1 drops=1`；
  随后呈现恢复。SDK 将 0x505 定义为 `GL_OUT_OF_MEMORY`，错误由呈现 context
  的 draw 后检查发现，现有证据不能定位到具体分配或证明系统内存耗尽。
  [GLES 验证记录](gles-direct-validation.md)的早期 EGL 基线也有同类错误，
  不能将其认定为 `-opengl` 新引入的专属问题。严格门禁未被放宽，原始失败保留。

两边都是 800×600 的兽族雨中基地，但不是固定视角/单位数对照：D3D8 收尾
选中萨尔、人口 18/30；GL 开始选中兵营、16/30，结束又移动视角、21/30。
电池温度分别 38→39℃ 与 36→38℃。因此不得将数值之比写成可复现收益率。
描述性 GL 统计直接解析同一份完整日志，包含错误附近窗口，没有裁掉卡顿或
覆盖 `result.json` 的拒绝结果。GL 期间还存在少量 CPU 底图更新，不能宣称所有
CPU 上传恒为零，更不能据此断言 Guest 完全不读回。

结论：游戏自身 GL 有实际局内提速迹象，与避开 WineD3D D3D8 读回/转换链的
方向一致，但没有单变量证明哪一段贡献了多少。D3D8 整帧 READONLY LockRect
仍是已明确定位的成本，不是可以删除的无用数据。暂不继续 War3 的深层同步、
JIT 或 DXVK-D3D8 实验，不新增默认策略或产品开关；后续按游戏参数选择 GL
仍需接受上述兼容性限制。公共 EGL 稳定性、其他后端与生命周期回归单独推进。
